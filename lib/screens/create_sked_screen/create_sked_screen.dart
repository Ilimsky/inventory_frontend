...
class _CreateSkedScreenState extends State<CreateSkedScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    // Для USER - автоматически выбираем его филиал
    if (authService.isUser && selectedDepartmentId == null) {
      WidgetsBinding.instance.addPostFrameCallback((_) {
        // Здесь нужно получить departmentId пользователя
        // Пока заглушка - нужно будет интегрировать с UserDepartmentBinding
        final userDepartmentId = _getUserDepartmentId(); // Этот метод нужно реализовать
        if (userDepartmentId != null) {
          setState(() {
            selectedDepartmentId = userDepartmentId;
            selectedDepartment = departmentProvider.departments
                .firstWhere((dept) => dept.id == userDepartmentId);
          });
        }
      });
    }
                  ...
                  child: Card(
                    child: Padding(
                      ...
                      DropdownButton<int>(
                        ...
                        onChanged: authService.isUser
                            ? null // USER не может менять филиал
                            : (newId) {
                          setState(() {
                        ...
                        items: departmentProvider.departments
                            .where((dept) => authService.isUser
                            ? dept.id == selectedDepartmentId // USER видит только свой филиал
                            : true // ADMIN/SUPERADMIN видят все
                        )
                            .map((dept) {

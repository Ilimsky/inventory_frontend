...
class SkedProvider extends ChangeNotifier {
  ...
  // В SkedProvider добавляем проверки в ключевые методы
  ...
  //   // Проверка прав доступа
  //   if (!authService.isSuperAdmin) {
  //     throw Exception('Доступ запрещен: списание доступно только супер-администраторам');
  //   }
  //
  //   try {
  //     _startLoading();
  ...
  Future<void> fetchSkedsByDepartment(int departmentId) async {
    ...
    // Проверка доступа USER к филиалу
    if (authService.isUser) {
      final userDepartmentId = await _getUserDepartmentId();
      if (userDepartmentId != departmentId) {
        throw Exception(
            'Доступ запрещен: можно просматривать только данные своего филиала');
      }
    }
  ...
  // В SkedProvider модифицируем методы загрузки данных
  ...
  Future<void> fetchAllSkedsPaged({
    ...
    try {
      ...
      // Для USER - загружаем только данные его филиала
      if (authService.isUser) {
        final userDepartmentId = await _getUserDepartmentId();
      ...
      // Для ADMIN/SUPERADMIN - загружаем все данные
  ...
  // В SkedProvider добавляем:
  Future<int?> _getUserDepartmentId() async {

    if (!authService.isUser) {
      return null; // Для ADMIN/SUPERADMIN не ограничиваем
    }

    // Здесь нужно интегрировать с вашей системой привязки пользователей к филиалам
    // Пока заглушка - нужно получить departmentId текущего пользователя
    try {
      // Пример: вызов API для получения departmentId пользователя
      // final userDepartment = await apiService.getUserDepartment();
      // return userDepartment?.departmentId;

      // Временная заглушка - возвращаем null (будет ограничивать доступ)
      return null;
    } catch (e) {
      debugPrint('Ошибка получения departmentId пользователя: $e');
      return null;
    }
  }
}
